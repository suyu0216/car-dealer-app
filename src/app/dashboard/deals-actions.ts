...
function parseDealForm(formData: FormData, canManageFinance: boolean) {
  ...
  // 2026-09-04 新增：金流架構 Phase 2——訂金/尾款選填指定是收進哪個
  // 金流帳戶（新版多帳戶架構，跟上面 payment_method 現金/銀行並存，不是
  // 取代關係）。實際存不存在／是否還在使用中留給 createDeal／updateDeal
  // 裡查資料庫驗證（見 validateAccountIds()），這裡只單純取值。
  const depositAccountId = optionalText(formData, "deposit_account_id");
  const balanceAccountId = optionalText(formData, "balance_account_id");
...
/** 2026-09-04 新增：確認合約表單送上來的訂金/尾款帳戶 id 真的是這個
 * 車行自己的、而且還在使用中——理由跟 company-expenses-actions.ts／
 * repair-items-actions.ts 的同一段檢查一樣。一次查完 deposit/balance
 * 兩個 id（去重），不用查兩次。 */
async function validateAccountIds(
  ...
  for (const id of uniqueIds) {
    if (!validIds.has(id)) return "選擇的收款帳戶不存在或已停用，請重新選擇。";
  }
  return null;
}
...
export async function createDeal(
  ...
  const accountError = await validateAccountIds(supabase, profile.tenant_id!, [
    values.deposit_account_id,
    values.balance_account_id,
  ]);
