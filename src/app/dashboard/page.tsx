...
import type { Car, Customer, Deal, FinancialAccount, Profile, RepairItem, TenantVideo, TradeInRequest } from "@/lib/supabase/types";
...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        ...
        // 2026-09-06（第三次）：補進 closed_commission_cost／
    ...
    supabase
      .from("repair_items")
      .select(
        "id, tenant_id, car_id, item_name, vendor_name, handler_name, amount, receipt_number, status, evidence_url, evidence_path, note, payment_method, account_id, reviewed_at, created_at"
      )
      .order("created_at", { ascending: false }),
    ...
    // 2026-09-04 新增：金流架構 Phase 2——請款撥款核准時要選帳戶，見
    // car-maintenance-tab.tsx 的 RepairItemRow／maintenance-module.tsx。
    // 只查啟用中的帳戶，停用的帳戶不該出現在「撥款要選哪個帳戶」這種
    // 新增資料的下拉選單裡（帳戶管理分頁本身另外查全部，含已停用的）。
    supabase
      .from("financial_accounts")
      .select("id, tenant_id, name, type, opening_balance, is_active, sort_order, note, created_at")
      .eq("is_active", true)
      .order("sort_order", { ascending: true }),
  ]);
  ...
  const financialAccountList = (financialAccounts ?? []) as FinancialAccount[];
  ...
  return (
    <div className="mx-auto min-h-screen max-w-6xl px-6 py-8">
      ...
      <DashboardShell
        ...
        financialAccounts={financialAccountList}
        staff={staffList}
