...
import type { Car, CompanyExpense, Deal, FinancialAccount, RepairItem, Tenant, Transaction, TransactionType } from "@/lib/supabase/types";
...
export function CashPoolModule({
  ...
  manualTransactions: Pick<
    Transaction,
    "id" | "type" | "amount" | "payment_method" | "date" | "category" | "note" | "voided_at" | "reversed_transaction_id"
  >[];
  /** 2026-08-31 新增：已核准的維修/整備請款撥款——「資金總覽」水池深度
   * 檢查後補上的第五個資料來源，見 cash-pool.ts 開頭的說明。 */
  repairItems: RepairItemSlice[];
  /** 2026-09-04 新增：手動記帳選帳戶用，見下面 ManualEntryPanel。 */
  ...
  return (
    <section className="space-y-6">
      ...
      <ManualEntryPanel onSaved={onDataChanged} financialAccounts={financialAccounts} />
...
function ManualEntryPanel({
  ...
  onSaved: () => void;
  /** 2026-09-04 新增：選填，這筆手動紀錄是哪個帳戶的收支，見下面表單裡
   * 的「帳戶」下拉選單——這筆紀錄可能是流入也可能是流出（上面已經有
   * 「收入／支出」選單決定方向），所以標「帳戶」不特別標「入帳」或
   * 「出帳」，避免跟另一個選單的方向意思重疊或矛盾（見 2026-09-05 安安
   * 反映的措辭修正）。 */
  financialAccounts: FinancialAccount[];
}) {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
      ...
      <form action={formAction} className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        {/* 2026-09-04 新增：選填，這筆手動紀錄是哪個金流帳戶的收支
            （新版多帳戶架構）——跟上面「歸類」現金/銀行並存，不是取代
            關係。 */}
        <div>
          <label className="block text-sm font-medium text-neutral-700">帳戶（選填）</label>
          <select name="account_id" defaultValue="" className={INPUT_CLASS}>
...
function CashPoolTimeline({
  ...
  // 2026-09-05：原本是直接刪除，改成作廢＋沖銷（跟新版「收支明細」同一套
  // 邏輯，見 cash-pool-actions.ts 的 voidManualCashTransaction 開頭說明）。
  async function handleVoid(sourceId: string) {
