...
import { IconInfo, IconEmptyState } from "./nav-icons";
...
export function ProfitShareModule({
  ...
  const result = useMemo(() => {
    const closedCars = cars.filter(
      (c) => c.status === "sold" && c.closed_at && monthKeyOf(c.closed_at) === month
    );
    const grossProfit = closedCars.reduce((sum, c) => {
    ...
    const opexRecords = monthExpenses.filter((e) => e.category !== "人事薪資");
    const opex = opexRecords.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);
    const payrollRecords = monthExpenses.filter((e) => e.category === "人事薪資");
    const basePayroll = payrollRecords.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);

    const netProfit = grossProfit - opex - basePayroll;

    // 2026-08-31 新增：分別記「這個月有沒有登記過任何一筆」，不是只看
    // 加總金額是不是 0——加總是 0 有兩種完全不同的情況：「這個月真的
    // 沒開銷」跟「公司帳務這個功能根本還沒登記過任何一筆資料」，這裡
    // 分開記下來，畫面上兩種要顯示的東西不一樣（見下面 SummaryCard 的
    // isEmpty），不然使用者看到的都是同一個「$0」，會誤以為系統壞掉。
    return {
      grossProfit,
      opex,
      basePayroll,
      netProfit,
      closedCarCount: closedCars.length,
      opexRecordCount: opexRecords.length,
      payrollRecordCount: payrollRecords.length,
      hasAnyExpenseEver: expenses.length > 0,
    };
  }, [cars, expenses, month]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2026-08-31 新增：說明橫幅——直接回應安安實際遇到的疑惑（「為啥
          營運費用沒有顯示，我都有開銷了？」）：她說的「開銷」其實是
          維修請款（整備/過戶/稅金），那筆費用已經算進車輛結帳總成本、
          折進「月毛利」裡了；這裡的「月營運費用／月人事底薪」只認
          「公司帳務」分頁裡另外登記的公司開銷（水電/租金/廣告/底薪
          這種跟單一台車無關的固定支出），兩邊是不同來源、不會互相
          顯示對方的數字，避免使用者以為系統漏算。 */}
      <div className="flex gap-3 rounded-2xl border border-blue-100 bg-blue-50/60 p-4">
        <span aria-hidden className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center text-blue-500">
          <IconInfo />
        </span>
        <p className="text-xs leading-relaxed text-blue-900/80">
          <span className="font-bold">月毛利</span>已經包含每台車的整備／過戶／稅金／業務抽成等結帳成本，不用再另外扣一次。
          <span className="font-bold">月營運費用</span>與<span className="font-bold">月人事底薪</span>
          只計算「公司帳務」分頁裡登記的公司開銷（例如水電、租金、廣告、員工底薪這類跟單一台車無關的支出）——
          如果這個月還沒去「公司帳務」登記任何一筆，這兩項就會顯示「尚無資料」，不是系統算錯或漏算。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          ...
          hint="已結案車輛：成交價－結帳總成本（已含抽成）"
          isEmpty={result.closedCarCount === 0}
          emptyHint="這個月還沒有已結案（售出）的車輛"
        />
        <SummaryCard
          label="月營運費用"
          value={-result.opex}
          hint="公司開銷，不含人事薪資"
          isEmpty={result.opexRecordCount === 0}
          emptyHint={
            result.hasAnyExpenseEver
              ? "這個月還沒登記公司開銷"
              : "還沒去「公司帳務」登記過公司開銷"
          }
        />
        <SummaryCard
          label="月人事底薪"
          value={-result.basePayroll}
          hint="公司開銷「人事薪資」類別"
          isEmpty={result.payrollRecordCount === 0}
          emptyHint={
            result.hasAnyExpenseEver
              ? "這個月還沒登記人事薪資開銷"
              : "還沒去「公司帳務」登記過人事薪資開銷"
...
function SummaryCard({
  ...
  hint?: string;
  emphasize?: boolean;
  /** 2026-08-31 新增：這個卡片對應的資料來源這個月「一筆都沒有」（不是
   * 加總金額剛好等於 0）——這時候不要顯示 "$0"，那看起來像是壞掉或
   * 漏算，改成跟 Hocar 報表頁一樣的空狀態圖示＋文字說明，讓使用者一眼
   * 就知道是「還沒登記」而不是「算錯了」。 */
  isEmpty?: boolean;
  emptyHint?: string;
}) {
    ...
    return (
      <div className="rounded-2xl border border-dashed border-neutral-200 bg-neutral-50 p-4">
        ...
        <div className="mt-1.5 flex items-center gap-1.5 text-neutral-400">
          <IconEmptyState className="h-4 w-4" />
          <span className={emphasize ? "text-lg font-bold" : "text-sm font-semibold"}>尚無資料</span>
        </div>
        {emptyHint && <p className="mt-1 text-[11px] text-neutral-400">{emptyHint}</p>}
      </div>
    );
  }
