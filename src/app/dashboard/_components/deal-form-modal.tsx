...
export function DealFormModal({
  ...
  /** 2026-09-04 新增：訂金/尾款選帳戶用，見下面 deposit_account_id／
   * balance_account_id 欄位。 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/50 px-4 py-8">
      ...
      >
        ...
        <form action={formAction} onChange={markDirty} onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          {/* 2026-09-04 新增：選填，訂金/尾款收進哪個金流帳戶（新版多帳戶
              架構）——跟上面的現金/匯款分類並存，不是取代關係，見
              financial-accounts-module.tsx。 */}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-neutral-700">訂金入帳帳戶</label>
              <select
                name="deposit_account_id"
                defaultValue={deal?.deposit_account_id ?? ""}
                className={INPUT_CLASS}
              >
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700">尾款入帳帳戶</label>
              <select
                name="balance_account_id"
                defaultValue={deal?.balance_account_id ?? ""}
                className={INPUT_CLASS}
              >
                <option value="">尚未指定</option>
                {financialAccounts.map((a) => (
