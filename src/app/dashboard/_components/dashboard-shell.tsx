...
import type { Car, Customer, Deal, FinancialAccount, RepairItem, Tenant, TenantVideo, TradeInRequest } from "@/lib/supabase/types";
...
export function DashboardShell({
  ...
  /** 2026-09-04 新增：金流架構 Phase 2——啟用中的帳戶清單，給「整備維修」
   * 分頁核准撥款時選帳戶用（見 maintenance-module.tsx／
   * car-maintenance-tab.tsx），也順帶傳給車輛庫存管理（車輛詳情頁的
   * 維修請款分頁同一套 RepairItemRow）。 */
  ...
  // 才退回「儀表板／總覽」——2026-09-04 起這個才是預設首頁，不再是車輛
  // 庫存管理（見 overview-module.tsx 開頭的說明）。車輛庫存管理現在跟其他
  // 分頁一樣，一定要帶 module=inventory 才會選到，見下面 modules 清單。
  const [active, setActiveState] = useState<ModuleKey>(() => parseModuleParam(moduleParam) ?? "overview");
  ...
  useEffect(() => {
    setActiveState(parseModuleParam(moduleParam) ?? "overview");
  }, [moduleParam]);
  ...
  const modules: ModuleKey[] = [
    // 「儀表板／總覽」一定顯示、不受任何權限限制——內容本身會依權限
    // 決定要不要顯示金額類卡片，見 overview-module.tsx 的 canViewAnalytics。
    "overview",
    "inventory",
    "maintenance",
    "crm",
  ...
  // 不會發生，但保險起見）而不再存在於清單裡，就退回儀表板總覽，
  // 避免畫面停留在一個已經不存在的分頁上。
  const activeModule = modules.includes(active) ? active : "overview";

  // 儀表板總覽的卡片/快速入口用這支切到其他分頁——改網址（router.push）
  // 而不是直接呼叫 setActiveState，理由見上面 useEffect 的說明：只改
  // client state、不改網址，會讓側邊欄的選中樣式、瀏覽器上一頁/重新整理
  // 都跟畫面實際顯示的分頁對不起來，統一走「改網址、側邊欄跟這支共用
  // 同一份 URL→activeModule 的推導邏輯」才不會又出現「畫面停在舊分頁」
  // 這類同步 bug。
  function navigateToModule(key: string) {
    router.push(`/dashboard?module=${key}`);
  }
