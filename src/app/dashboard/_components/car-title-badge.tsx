 ...
 * 登記在二胎公司/人頭名下、還是根本還沒過戶到公司（剛入庫），不用點進
 * 詳情或編輯表單才知道。跟收購進價一樣算敏感財務資訊，只有 canViewCost
 * 看得到，見 car-form-modal.tsx「二胎／人頭車合作紀錄」區塊的權限說明
 * ——沒有這個權限的人，這個徽章直接不顯示（不是顯示鎖頭），避免庫存
 * 列表的徽章列一直出現使用者看不懂也用不到的鎖頭圖示。
 *
 * 三種狀態的優先順序（2026-09-04 新增第三種）：
 * 1. isCurrentlyOutOnNominee() 為 true → 紅色警示，優先顯示，不管
 *    title_at_company 勾選與否（都登記在別人名下了，車籍當然不在公司）。
 * 2. title_at_company 為 true → 綠色「車籍在公司」。
 * 3. 其餘情況（沒有二胎/人頭，但 title_at_company 還沒勾）→ 新增的
 *    灰色/中性徽章「車籍尚未到公司」——典型情境是剛入庫、過戶還在
 *    辦理中，避免使用者看到誤導的綠色徽章。 */
export function CarTitleBadge({
  ...
  if (!car.title_at_company) {
    return (
      <span
        className={
          "inline-flex items-center whitespace-nowrap rounded-full bg-[#F2F0EC] px-2.5 py-0.5 text-xs font-medium text-[#8A7F6A] ring-1 ring-inset ring-[#E3DED3]" +
          (className ? ` ${className}` : "")
        }
        title="車籍尚未過戶到公司名下（例如剛入庫、過戶還在辦理中）"
      >
        🕒 車籍尚未到公司
      </span>
    );
