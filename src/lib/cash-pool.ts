...
export interface CashPoolEvent {
   ...
   * 資料用途，畫面上還沒做跳轉連結。 */
  sourceId: string;
  /** 2026-09-05 新增：只有 kind === "manual" 才可能有值，跟 financial-
   * ledger.ts 的 LedgerEvent.isVoided/isReversal 是同一個概念——這裡跟
   * 新版多帳戶收支明細讀的是同一張 transactions 表，作廢＋沖銷之後這個
   * 舊版「資金總覽」時間軸也會看到同一組多出來的兩筆，必須一樣標示清楚，
   * 不然使用者會看到兩筆金額相反、原因不明的手動紀錄。 */
  isVoided?: boolean;
  isReversal?: boolean;
}
...
export function computeCashPoolSummary(params: {
  ...
  for (const item of manual) {
    ...
    events.push({
      ...
      sourceId: item.id,
      isVoided: !!item.voided_at,
      isReversal: !!item.reversed_transaction_id,
    });
  }
