...
export const getCurrentProfile = cache(async () => {
  ...
  const { data: profile, error } = await supabase
    .from("profiles")
    .select(
      "id, tenant_id, role, name, can_view_cost, can_view_salary, can_edit_cars, public_phone, public_line_id, show_public_contact, public_bio, public_avatar_url, created_at"
    )
    .eq("id", user.id)
    .single();
